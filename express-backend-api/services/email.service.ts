...
export class EmailService {
  ...
  public async sendEmailConfirmationMail(user: User): Promise<void> {
    ...
    let info = await this.transporter.sendMail({
      ...
      html: '<p>Click <a href="http://localhost:4200/confirm/' + user.emailToken + '"> here</a> to verify your account.</p>', // html body
    });
  }
  ...
  public async sendPasswordResetMail(user: User): Promise<void> {
    let info = await this.transporter.sendMail({
      ...
      html: '<p>Click <a href="http://localhost:4200/reset/' + user.emailToken + '"> here</a> to verify your account.</p>', // html body
    });
  }
}
